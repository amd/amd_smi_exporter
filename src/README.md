1. [Introduction to the AMD Prometheus Exporter](#desc)
...
8. [Objects exported by the AMD Prometheus Exporter](#objects)
a. [At the Core level](#core)
b. [At the Socket level](#socket)
c. [At the System level](#system)
d. [Custom rules](#custom)

<a name="desc"></a>
# AMD EPYC™ Prometheus Exporter

The AMD EPYC™ Prometheus Exporter is a standalone app that can be run as a daemon, written in GO Language, that exports AMD EPYC™ CPU metrics to the Prometheus server. The AMD Prometheus Exporter employs the [E-SMI In-Band C library](https://github.com/amd/e_smi_library.git) for its data acquisition. The exporter and the E-SMI library have a [GO binding](https://github.com/amd/goesmi.git) that provides an interface between the e-smi C library and the GO exporter code. 

## Important note about Versioning and Backward Compatibility
The AMD EPYC™ Prometheus Exporter follows the E-SMI In-band library in its releases, as it is dependent on the underlying E-SMI library for its data. The Exporter is currently under development, and therefore subject to change in the features it offers and at the interface with the GO binding. 
...
The source code for the GO Exporter is available at [amd_prom_exporter](https://github.com/amd/amd_prom_exporter.git).
...
Before executing the GO exporter as a standalone executable or as a service, one needs to ensure that the e-smi library dependencies are met by ensuring that they are installed in the "/opt/esmi" directory. Please refer to <https://github.com/amd/esmi_ib_library/docs/README.md> for the build and installation instructions.
...
<a name="objects"></a>
# Objects exported by the AMD Prometheus Exporter
...
## Curl commands to query the prometheus exporter

* > ### curl <Prometheus Client IP>:2021/amd_num_sockets

	*NOTE:* The the port number of the prometheus client depends on the configuration setting. It is set to 2021 by default.
